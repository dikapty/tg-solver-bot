...
- **Защита**: дневной лимит на пользователя (`DAILY_LIMIT`), один одновременный запрос на пользователя (asyncio.Lock + антиспам-интервал), белый список админов (`/admin_stats`).
...
| `/admin_stats` | Только для `ADMIN_IDS`: число пользователей, запросы за день |
